...
:::{warning}
The material composite panels on this page are reproduced from the original research notebooks. Those notebooks still use a PyVista manuscript-export style and depend on the notebook-only `NodalSkeletonMultiBand` helper, so the PNG panels are not currently regenerated by the public package during the documentation build. The public downstream API shown here remains the same: once the surface and spatial graph exist, use `plot_3D_graph_plotly`, projection, PD-code, and Yamada tools.
:::
...
For the framework-focused notebook figure map, see
[Notebook Figure Gallery](paper_notebook_gallery.md). That page keeps only the
notebook figures that directly teach surface, skeleton, and spatial-graph
objects.
...
Input source: a DFT-fitted two-band material Hamiltonian.
...
Display the produced stages in the same order used by the library: Hamiltonian
window, surface, spatial graph, then invariant.

```python
# Paper-notebook helper: each window uses the same Hamiltonian with a different
# energy filter before the surface is extracted.
ti3al_records = []
for window_name, window in [
    ("E < 0.25 eV", dict(energy_min=None, energy_max=0.25)),
    ("0.25 eV < E", dict(energy_min=0.25, energy_max=None)),
]:
    ske = NodalSkeletonMultiBand(
        H_Ti3Al_sympy(),
        k_symbols=(kx, ky, kz),
    ...
    surface = ske.exceptional_surface_pv.connectivity("largest")
    graph = ske.skeleton_graph(simplify=True)
    ti3al_records.append((window_name, surface, graph))
...
fig = make_subplots(
    rows=2,
    cols=len(ti3al_records),
    specs=[
        [{"type": "scene"} for _ in ti3al_records],
        [{"type": "scene"} for _ in ti3al_records],
    ],
...
for col, (window_name, surface, graph) in enumerate(ti3al_records, start=1):
...
for index in range(1, 2 * len(ti3al_records) + 1):
...
```{image} ../assets/plot_outputs/material_ti3al_transition.png
:alt: Output from plotting Ti3Al surfaces and extracted graph stages
:width: 88%
:align: center
```
...
Input source: a three-band $D_6$-symmetric material Hamiltonian from the
manuscript notebook.
...
Display the material stages by keeping the extracted surface and graph together
for each energy window:

```python
# Paper-notebook helper: one NodalSkeletonMultiBand object per energy window.
tib2_records = []
for window_name, window in [
    ("E < 2.8 eV", dict(energy_min=None, energy_max=2.8)),
    ("E ~ 2.8 eV", dict(energy_min=2.75, energy_max=2.85)),
    ("2.8 eV < E < 3.9 eV", dict(energy_min=2.8, energy_max=3.9)),
    ("3.9 eV < E", dict(energy_min=3.9, energy_max=None)),
]:
    ske = NodalSkeletonMultiBand(
        H_D6_sympy(params),
        k_symbols=(kx, ky, kz),
    ...
    surface = ske.exceptional_surface_pv.connectivity("largest")
    graph = ske.skeleton_graph(simplify=True)
    tib2_records.append((window_name, surface, graph))
...
fig = make_subplots(
    rows=2,
    cols=len(tib2_records),
    specs=[
        [{"type": "scene"} for _ in tib2_records],
        [{"type": "scene"} for _ in tib2_records],
    ],
    horizontal_spacing=0.02,
    vertical_spacing=0.02,
...
for col, (window_name, surface, graph) in enumerate(tib2_records, start=1):
    mesh = surface.triangulate()
    faces = mesh.faces.reshape(-1, 4)[:, 1:]
    pts = mesh.points
    fig.add_trace(
...
for index in range(1, 2 * len(tib2_records) + 1):
    scene_name = "scene" if index == 1 else f"scene{index}"
    fig.update_layout(
        **{
            scene_name: dict(
...
```{image} ../assets/plot_outputs/material_tib2_transition.png
:alt: Output from plotting TiB2 material surfaces and extracted graph stages
:width: 92%
:align: center
```
...
Input source: a symmetry-guided two-band model for the low-energy nodal
structure of `YH3`.
...
surface-to-graph transition is still the same library pattern:

```python
# Paper-notebook helper: one surface/graph record per energy window.
yh3_records = []
for window_name, window in [
    ("E < 0.006 eV", dict(energy_min=None, energy_max=0.006)),
    ("0.006 eV < E < 0.2 eV", dict(energy_min=0.006, energy_max=0.2)),
    ("0.2 eV < E", dict(energy_min=0.2, energy_max=None)),
]:
    ske = NodalSkeletonMultiBand(
        H_YH3_sympy(),
        k_symbols=(kx, ky, kz),
    ...
    surface = ske.exceptional_surface_pv.connectivity("largest")
    graph = ske.skeleton_graph(simplify=True)
    yh3_records.append((window_name, surface, graph))
...
fig = make_subplots(
    rows=2,
    cols=len(yh3_records),
    specs=[
        [{"type": "scene"} for _ in yh3_records],
        [{"type": "scene"} for _ in yh3_records],
    ],
    horizontal_spacing=0.02,
    vertical_spacing=0.02,
...
for col, (window_name, surface, graph) in enumerate(yh3_records, start=1):
    mesh = surface.triangulate()
    faces = mesh.faces.reshape(-1, 4)[:, 1:]
    pts = mesh.points
    fig.add_trace(
...
for index in range(1, 2 * len(yh3_records) + 1):
    scene_name = "scene" if index == 1 else f"scene{index}"
    fig.update_layout(
        **{
            scene_name: dict(
...
```{image} ../assets/plot_outputs/material_yh3_transition.png
:alt: Output from plotting YH3 material surfaces and extracted graph stages
:width: 92%
:align: center
```
...
Input source: a six-band tight-binding Hamiltonian fitted for `Co2MnGa`.
...
transition plot below shows the essential order: material Hamiltonian, extracted
surface, spatial graph, and then the Yamada fingerprint printed above.
...
for window_name, window in [
    ("E < 0.25 eV", dict(energy_min=None, energy_max=0.25)),
    ("0.25 eV < E < 1.15 eV", dict(energy_min=0.25, energy_max=1.15)),
    ("1.15 eV < E", dict(energy_min=1.15, energy_max=None)),
]:
    ske = NodalSkeletonMultiBand(
        H_Co2MnGa_TB6_sympy(),
        k_symbols=(kx, ky, kz),
    ...
    surface = ske.exceptional_surface_pv.connectivity("largest")
    graph = ske.skeleton_graph(simplify=True)
    co2mnga_records.append((window_name, surface, graph))
...
fig = make_subplots(
    rows=2,
    cols=len(co2mnga_records),
    specs=[
        [{"type": "scene"} for _ in co2mnga_records],
        [{"type": "scene"} for _ in co2mnga_records],
    ],
    horizontal_spacing=0.02,
    vertical_spacing=0.02,
...
for col, (window_name, surface, graph) in enumerate(co2mnga_records, start=1):
    mesh = surface.triangulate()
    faces = mesh.faces.reshape(-1, 4)[:, 1:]
    pts = mesh.points
    fig.add_trace(
...
for index in range(1, 2 * len(co2mnga_records) + 1):
    scene_name = "scene" if index == 1 else f"scene{index}"
    fig.update_layout(
        **{
            scene_name: dict(
...
```{image} ../assets/plot_outputs/material_co2mnga_transition.png
:alt: Output from plotting Co2MnGa material surfaces and extracted graph stages
:width: 92%
:align: center
```
...
The multiband paper figures are now documented in
[Notebook Figure Gallery](paper_notebook_gallery.md).  Use this page for the
main material tutorial and the gallery page for the full paper-figure audit.

The remaining library cleanup is to promote the research-notebook
`NodalSkeletonMultiBand` helper into `knotted_graph.applications`.  The
downstream public pattern is already the same in every material example:
Hamiltonian, surface extraction, graph extraction, projection or PD code, and
$\Upsilon(G;Y)$ / $\Upsilon_{\partial F}$.
