...
namespace LWS {
    ...
    Vector3 TPESC::tpe_grad_Kf(const TangentMassPoint& i, CurveVertex* j, double alpha, double beta, CurveVertex* wrt) {
        // Get positions and displacement vectors
    ...
    Vector3 TPESC::tpe_grad_Kf(CurveVertex* i, const TangentMassPoint& j, double alpha, double beta, CurveVertex* wrt) {
        // Get positions and displacement vectors
    ...
    Vector3 TPESC::tpe_grad(const TangentMassPoint& x, CurveVertex* y, double alpha, double beta, CurveVertex* wrt) {
        // Computes the gradient of the kernel (K_f(x, y) dx dy) with respect to
        // the position of the vertex "wrt".
    ...
    Vector3 TPESC::tpe_grad(CurveVertex* x, const TangentMassPoint& y, double alpha, double beta, CurveVertex* wrt) {
    ...
    Vector3 TPESC::grad_norm_proj_alpha(const TangentMassPoint& i, CurveVertex* j, double alpha, double beta, CurveVertex* wrt) {
        Vector3 disp = i.point - j->Position();
        Vector3 T_i = i.tangent;
        // Projection onto normal plane
        Vector3 normal_proj = disp - dot(disp, T_i) * T_i;
    ...
    Vector3 TPESC::grad_norm_proj_alpha(CurveVertex* i, const TangentMassPoint& j, double alpha, double beta, CurveVertex* wrt) {
        Vector3 disp = i->Position() - j.point;
        Vector3 T_i = i->Tangent();
        // Projection onto normal plane
        Vector3 normal_proj = disp - dot(disp, T_i) * T_i;
    ...
    VertJacobian TPESC::grad_tangent_proj(const TangentMassPoint& i, CurveVertex* j, CurveVertex* wrt) {
        // Differentiate the inner product
        Vector3 disp = i.point - j->Position();
        Vector3 T_i = i.tangent;
        double disp_dot_T = dot(disp, T_i);
    ...
    VertJacobian TPESC::grad_tangent_proj(CurveVertex* i, const TangentMassPoint& j, CurveVertex* wrt) {
        // Differentiate the inner product
        Vector3 disp = i->Position() - j.point;
        Vector3 T_i = i->Tangent();
        double disp_dot_T = dot(disp, T_i);
