...
namespace LWS {
    ...
    inline double GetCoordFromBody(const VertexBody6D& body, int axis) {
        switch (axis) {
    ...
    BVHNode3D::BVHNode3D(std::vector<VertexBody6D> &points, int axis, BVHNode3D* root, bool splitTangents) {
        ...
        // except set the fields from that one point
            ...
            std::vector<VertexBody6D> sortedPoints = points;
            std::sort(sortedPoints.begin(), sortedPoints.end(),
                [axis](const VertexBody6D& a, const VertexBody6D& b) {
                    return GetCoordFromBody(a, axis) < GetCoordFromBody(b, axis);
                });

            size_t splitIndex = 0;
            double minWidths = INFINITY;
            const double firstCoord = GetCoordFromBody(sortedPoints.front(), axis);
            const double lastCoord = GetCoordFromBody(sortedPoints.back(), axis);

            for (size_t i = 0; i + 1 < sortedPoints.size(); i++) {
                const double width1 = GetCoordFromBody(sortedPoints[i], axis) - firstCoord;
                const double width2 = lastCoord - GetCoordFromBody(sortedPoints[i + 1], axis);

                const double sumSquares = width1 * width1 + width2 * width2;
            ...
            splitPoint = (GetCoordFromBody(sortedPoints[splitIndex], axis)
                + GetCoordFromBody(sortedPoints[splitIndex + 1], axis)) / 2.0;

            const size_t midpoint = splitIndex + 1;
            lesserPoints.assign(sortedPoints.begin(), sortedPoints.begin() + midpoint);
            greaterPoints.assign(sortedPoints.begin() + midpoint, sortedPoints.end());
    ...
    double BVHNode3D::AxisSplittingPlane(std::vector<VertexBody6D> &points, int axis) {
        ...
        for (size_t i = 0; i + 1 < nPoints; i++) {
            ...
            double width2 = coords[nPoints - 1] - coords[i + 1];
