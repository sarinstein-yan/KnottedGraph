...
def _random_prepared(seed: int, crossing_count: int):
    ...
    # Crossing ports contribute 4c endpoints. Add an even number of fixed
    # terminal endpoints so a physical-arc perfect matching always exists.
...
def test_frontier_value_is_independent_of_factor_elimination_order():
    for crossing_count in (1, 2, 3, 4):
        for offset in range(8):
            ...
            actual = compute_diagram_frontier_laurent(
                prepared,
                factor_order=reversed_order,
            )
            assert actual == expected
...
def test_frontier_limits_are_performance_guards_only():
    ...
    # A deliberately tiny dynamic limit must abort rather than return a partial
    # polynomial. Production catches this and uses another exact backend.
    with pytest.raises(FrontierLimitExceeded):
