...
def test_dispatched_compact_yamada_matches_python_exact_fallback():
    A = sp.Symbol("A")
    python_exact = PythonCompactYamadaEvaluator()
    for graph in _suite():
        _equal(
            CompactYamadaEvaluator().compute(graph, A), python_exact.compute(graph, A)
        )


def test_dispatched_compact_negami_specialization_matches_python_exact_fallback():
    A = sp.Symbol("A")
    python_exact = PythonCompactNegamiSpecializedEvaluator()
    for graph in _suite():
        _equal(
            CompactNegamiSpecializedEvaluator().compute(graph, A),
