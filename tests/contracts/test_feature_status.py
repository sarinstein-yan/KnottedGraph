...
EXPECTED_ROUTE_FIELDS = {
    ...
    "Named knot/link, torus type, or Artin braid": (
        "Public · base",
        "none",
        "`knotted_graph.inputs.KnotFunction`",
        "`KnotFunction`",
    ),
    "Analytic knot field needing a tubular handlebody or spatial graph": (
        "Public · optional",
        "`knot-fields`",
        "`knotted_graph.inputs.KnotFunction.to_spatial_graph`",
        "`networkx.MultiGraph`",
    ),
    ...
    "Deformation between two analytic knot fields": (
        "Application API · optional",
        "`knot-fields`",
        "`knotted_graph.applications.knot_deformation.KnotDeformationScan`",
    ...
    "Knot-field or in-memory Hamiltonian Yamada phase map": (
        "Application API · optional",
        "`knot-fields` for knot sources; `nodal` for Hamiltonian/material sources",
...
OPTIONAL_CALLS = [
    (
        "knotted_graph.applications.knot_deformation",
        "KnotDeformationScan",
        ("skimage",),
    ),
    (
...
def test_documented_knot_field_graph_route_is_public():
    module = importlib.import_module("knotted_graph.inputs")
    assert callable(module.KnotFunction.to_spatial_graph)
