...
def measure_case(name, graph, rotation, expected_crossings):
    ...
    if crossings != expected_crossings:
        raise AssertionError(
            f"{name}: expected {expected_crossings} crossings, got {crossings}"
        )
    expected_blocks = 5 if name == "decomposable_c5" else 1
    if len(Yamada.from_PDCode(processor)._diagram_blocks()) != expected_blocks:
        raise AssertionError(f"{name}: unexpected block factorization")

    invariant_s, polynomial = median(
        lambda: Yamada.from_PDCode(processor).compute(
            A,
            normalize=False,
            n_jobs=1,
            method="negami",
    ...
    def complete():
        projected = PDCode(graph)
        projected.compute(rotation_angles=rotation)
        return Yamada.from_PDCode(projected).compute(
            A,
            normalize=False,
            n_jobs=1,
            method="negami",
