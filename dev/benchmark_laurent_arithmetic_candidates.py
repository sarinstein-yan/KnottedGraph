...
def arithmetic_rows():
    ...
    for length in (5, 15, 40, 100):
        ...
        for name, base_fn, candidate_fn, args in (
            ...
            baseline_t, baseline = med(
                lambda: base_fn(*args),
                repeats=1200 if name == "multiply" else 4000,
            )
            candidate_t, candidate = med(
                lambda: candidate_fn(*args),
                repeats=1200 if name == "multiply" else 4000,
            )
            if baseline != candidate:
                raise AssertionError(
                    f"Laurent arithmetic mismatch: {name} length={length}"
                )
            row = {
                "operation": name,
                "terms": length,
                "baseline": "current_python_laurent_ops",
...
class CandidateEvaluator:
        ...
        if edge_count == 0:
            ...
            if len(components) > 1:
                ...
                for component in components:
                    value = candidate_multiply(
                        value, self.rec(graph.induced(component))
                    )
            elif (
                graph.n == 2
                and not graph.rows[0][0]
                and not graph.rows[1][1]
                and graph.rows[0][1] == edge_count
            ):
                value = ZERO
                power = ONE
                ...
                if bridge:
                    ...
                    value = candidate_multiply_sigma(
                        self.rec(graph.delete_loop(loop)), sign=-1
                    )
    ...
    def _branch(self, graph, edge):
        ...
        return candidate_add(
            self.rec(graph.delete_edge(i, j)),
            self.rec(graph.contract_edge(i, j)),
        )
...
def real_kernel_rows():
    ...
    for name, graph in cases:
        ...
        baseline_t, baseline = med(
            lambda: PythonCompactYamadaEvaluator().compute_laurent(compact),
            repeats=300,
        )
        candidate_t, candidate = med(
            lambda: CandidateEvaluator().rec(compact), repeats=300
        )
        if baseline != candidate:
